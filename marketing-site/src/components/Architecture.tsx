        ...
        <div className="mx-auto mt-16 max-w-3xl">
          <div className="flex flex-col gap-2">
            {LAYERS.map((layer, idx) => (
              <div key={layer.label}>
                <div className="flex items-start gap-5 rounded-xl border border-slate-200 bg-slate-50 px-6 py-5">
                  <span className="mt-0.5 shrink-0 text-xs font-bold tracking-widest text-brand-600">
                    {layer.num}
                  </span>
                  <div>
                    <h3 className="text-sm font-bold text-slate-900">{layer.label}</h3>
                    <p className="mt-1 text-sm leading-relaxed text-slate-500">{layer.desc}</p>
                  </div>
                </div>
                {idx < LAYERS.length - 1 && (
