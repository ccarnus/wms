...
- The left sidebar switches between views (`Dashboard`, `Labor`, `Inventory`, `Shipments`, `Configuration`, `Users`, `Integrations`). Operators bypass the sidebar entirely and get a full-screen task interface.
- All data tables share a common UI kit (`frontend/src/components/ui.jsx`): click a column header to sort (ascending → descending → off), use the toolbar to search and filter by field, and page through long lists. The Dashboard view shows live KPIs, inventory alerts, and sortable Open Tasks / Sales Orders tables.
